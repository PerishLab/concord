...
async fn activity() {
    ...
    let first = estate
        .touch(&task, &codex, "task.show")
        .expect("record first touch");
    assert!(first.recent.is_empty());
    let repeated = estate
        .touch(&task, &codex, "phase.list")
    ...
    let concurrent = estate
        .touch(&task, &claude, "member.status")
        .expect("record other session touch");
    assert_eq!(concurrent.recent.len(), 1);
    assert_eq!(concurrent.recent[0].agent, Agent::Codex);
    ...
    assert_eq!(value["touches"].as_array().expect("touches").len(), 2);
    #[cfg(unix)]
    {
